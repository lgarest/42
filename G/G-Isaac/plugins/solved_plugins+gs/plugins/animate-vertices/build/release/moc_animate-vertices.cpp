/****************************************************************************
** Meta object code from reading C++ file 'animate-vertices.h'
**
** Created: Mon Nov 18 19:31:51 2013
**      by: The Qt Meta Object Compiler version 63 (Qt 4.8.1)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../../include/animate-vertices.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'animate-vertices.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 63
#error "This file was generated using the moc from 4.8.1. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_AnimateVertices[] = {

 // content:
       6,       // revision
       0,       // classname
       0,    0, // classinfo
       0,    0, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       0,       // signalCount

       0        // eod
};

static const char qt_meta_stringdata_AnimateVertices[] = {
    "AnimateVertices\0"
};

void AnimateVertices::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    Q_UNUSED(_o);
    Q_UNUSED(_id);
    Q_UNUSED(_c);
    Q_UNUSED(_a);
}

const QMetaObjectExtraData AnimateVertices::staticMetaObjectExtraData = {
    0,  qt_static_metacall 
};

const QMetaObject AnimateVertices::staticMetaObject = {
    { &QObject::staticMetaObject, qt_meta_stringdata_AnimateVertices,
      qt_meta_data_AnimateVertices, &staticMetaObjectExtraData }
};

#ifdef Q_NO_DATA_RELOCATION
const QMetaObject &AnimateVertices::getStaticMetaObject() { return staticMetaObject; }
#endif //Q_NO_DATA_RELOCATION

const QMetaObject *AnimateVertices::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->metaObject : &staticMetaObject;
}

void *AnimateVertices::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_AnimateVertices))
        return static_cast<void*>(const_cast< AnimateVertices*>(this));
    if (!strcmp(_clname, "EffectInterface"))
        return static_cast< EffectInterface*>(const_cast< AnimateVertices*>(this));
    if (!strcmp(_clname, "edu.upc.fib.graug.EffectInterface/1.0"))
        return static_cast< EffectInterface*>(const_cast< AnimateVertices*>(this));
    return QObject::qt_metacast(_clname);
}

int AnimateVertices::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QObject::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    return _id;
}
QT_END_MOC_NAMESPACE
