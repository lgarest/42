...
// Returns if the propagation gives conflict
{
	...
	while ( indexOfNextLitToPropagate < modelStack.size() ) {
		...
		for (uint i = 0; i < clausesVectorSize; ++i) {
			...
			// clauseIter == iterator over the positive or negative clauses vector where the lit appears
			int clauseIter;
	      	if (lastDecision > 0)
			...
			if (not someLitTrue and numUndefs == 0)
				...
				++propagations; // increase # of propagations
		}
	}
	return false;
}
...
int main(){
	...
	for (uint i = 0; i < numClauses; ++i)
		if (clauses[i].size() == 1) {
			...
			if (val == FALSE) {
				...
				cout << timer << " " << propagations << " ";
				cout << decisions << " UNSATISFIABLE" << endl;
				return 10;
			}
	...
	while (true) {
		...
		if (decisionLit == 0) {
			...
			double timer = (double) (clock() - tStart)/CLOCKS_PER_SEC;
			cout << timer << " " << propagations << " ";
			cout << decisions << " SATISFIABLE" << endl;
			return 20;
		}
		...
		++indexOfNextLitToPropagate;
		++decisionLevel;
		++decisions; // increase # of decisions
